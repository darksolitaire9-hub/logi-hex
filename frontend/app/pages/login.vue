...
import { ref } from "vue";
import { loginApi } from "../../lib/api/logiHex";
...
<template>
  <div class="w-full max-w-sm mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden">
    ...
    <form class="px-6 py-6 flex flex-col gap-5" @submit.prevent="handleLogin" novalidate>
      <div
        v-if="error"
        role="alert"
      ...
      <div>
        <label for="username" class="block text-sm font-medium text-[#1a1a2e] mb-1.5">
          Username
        </label>
        <input
          id="username"
      ...
      <div>
        <label for="password" class="block text-sm font-medium text-[#1a1a2e] mb-1.5">
          Password
        </label>
        <input
          id="password"
