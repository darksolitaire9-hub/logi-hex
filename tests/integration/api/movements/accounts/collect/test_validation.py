...
@pytest.mark.anyio
async def test_empty_items_list_returns_400_for_collect(api_client, db_session):
...
@pytest.mark.anyio
async def test_missing_client_id_returns_422_for_collect(api_client, db_session):
...
@pytest.mark.anyio
async def test_missing_items_field_returns_422_for_collect(api_client, db_session):
    workspace, client, _ = await seed_workspace_graph(db_session)
